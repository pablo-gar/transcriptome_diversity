...
# Expression association dirs
expression_association_gtex_dir <- '/scratch/users/paedugar/cnv_gtex_project/transcriptome_diversity/expression_associations/'
expression_association_others_dir <- '/scratch/users/paedugar/cnv_gtex_project/transcriptome_diversity_other_datasets/expression_associations/'
```
...
# Results expression associations
Load data
```{r}

# All datasets
expression_association_others <- read_expression_associations(expression_association_others_dir, 'other_datasets', '.txt')
expression_association_others$r2[is.infinite(expression_association_others$statistic)] <- 1
expression_association_others <- dplyr::filter(expression_association_others, statistic != 0)
```

Table of summary statistic for variance explained
```{r}
var_explained_others <- expression_association_others %>%
    group_by(id_names) %>%
    summarise(mean_r2 = mean(r2), median_r2 = median(r2), first_quartile = quantile(r2, 0.25), third_quartile = quantile(r2, 0.75)) %>%
    ungroup() %>%
    arrange(desc(mean_r2))

var_explained_others %>%
    kable() %>%
    kable_styling()
```
...
# TCGA
cors_other <- read_cors(PCA_out_dir_other, 'all', 'cors.txt')
cors_other$project[grepl('arora', cors_other$id_names)] <- sapply(strsplit(cors_other$id_names[grepl('arora', cors_other$id_names)], '_'), function(x) paste0(x[1:3], collapse='_'))

pca_stats_other <-read_cors(PCA_out_dir_other, 'all', 'PC_stats')
pca_stats_other$project[grepl('arora', pca_stats_other$id_names)] <- sapply(strsplit(pca_stats_other$id_names[grepl('arora', pca_stats_other$id_names)], '_'), function(x) paste0(x[1:3], collapse='_'))
...
pca_stats_all <-pivot_longer(pca_stats_other, starts_with('PC'), names_to='feature', values_to='stat_value')
...
read_expression_associations <- function(pc_dir, project, pattern) {
    
    cor_files <- list.files(pc_dir, full.names=T, recursive=T, pattern=pattern)
    cor_ids <- basename(cor_files)
    cors <- concatenate_table_files2(cor_files, id_names=cor_ids, col_types='ccdddddd')
    cors$project <- project
