...
main <- function(cmdArgs=commandArgs(T)) {
    ...
    # Bind results
    eqtl_results <- bind_rows(eqtl_trans, eqtl_trans_no_peer, eqtl_no_trans, eqtl_peer, eqtl_no_peer, eqtl_peer_plus)
    eqtl_results$group <- factor(eqtl_results$group, levels=c('no_covariates', 'transcriptome_diversity_as_covariates', 'no_peer', 'transcriptome_diversity_as_covariates_plus_nonPeer', 'peer', 'peer_plus'))
...
read_eqtl <- function(eqtl_dir, group) {
    ...
    cors <- concatenate_table_files2(files, id_names=ids, delim=' ', header=F, read_function=read_delim, progress=F, col=cols())
    cors$group <- group
